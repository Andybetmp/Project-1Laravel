        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('productos.store') }}" method="POST">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="text" id="nombre" name="nombre" value="{{ old('nombre') }}" style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 6px;">
                </div>
                <div style="margin-bottom: 15px;">
                    <label for="descripcion" style="display: block; font-weight: bold; margin-bottom: 5px;">Descripción :</label>
                    <textarea  id="descripcion" name="descripcion" rows="3" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px;">{{ old('descripcion') }}</textarea>
                </div>
                <div style="margin-bottom: 15px;">
                    <label for="precio" style="display: block; font-weight: bold; margin-bottom: 5px;">Precio: (S/.)</label>
                    <input type="text" id="precio" step="0.01" name="precio" value="{{ old('precio') }}" style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 6px;">
                </div>
                <div style="margin-bottom: 15px;">
                    <label for="stock" style="display: block; font-weight: bold; margin-bottom: 5px;">Stock disponible:</label>
                    <input type="text" id="stock" name="stock" value="{{ old('stock') }}" style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 6px;">
                </div>

                <button type="submit" style="background-color: #2563eb; color: white; padding: 10px 20px; border-radius: 6px; font-weight: bold; cursor: pointer;">Agregar Producto</button>
            </form>
        </div>
    </div>
</x-app-layout>